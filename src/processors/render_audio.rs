...
pub unsafe fn render_audio(plugin: &mut Plugin, input: *const f32, output: *mut f32, nframes: usize) {
    ...
    for i in 0..nframes {
        plugin.input_conv_buf[i] = input_slice[i] as f64 * input_gain;
    }

    if let Some(model) = plugin.nam.model.as_mut() {
        nam::nam::ffi::dsp_process(
            model.pin_mut(),
            &plugin.input_conv_buf[..nframes],
            &mut plugin.output_conv_buf[..nframes],
        );

        for i in 0..nframes {
            output_slice[i] = (plugin.output_conv_buf[i] * output_gain) as f32;
        }
    } else {
